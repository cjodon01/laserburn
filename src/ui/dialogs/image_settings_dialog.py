...
class ImagePreviewWidget(QLabel):
    ...
    def set_image(self, image: np.ndarray):
        """Set image from numpy array (grayscale or binary)."""
        if image is None:
            self.clear()
            return
...
class ImageSettingsDialog(QDialog):
    ...
    def _process_image(self) -> np.ndarray:
        ...
        result = ditherer.dither(img, threshold)
    ...
    def _update_info_label(self, image: np.ndarray, processed: bool):
        ...
        if processed:
            ...
            info = (
                ...
                f"Fill: {fill_percent:.1f}% ({black_pixels:,} pixels)"
            )
        else:
            info = (
                f"{status}: {width} x {height} px\n"
                f"Will engrave at: {width_mm:.1f} x {height_mm:.1f} mm"
            )
    ...
    def _apply_settings(self):
        ...
        self.image_shape.dpi = self.dpi_spin.value()
